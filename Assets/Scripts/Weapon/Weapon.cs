...
public class Weapon: Pickup
{
    ...
    private readonly int pickupLayer = 8;
    private readonly int ignoreLayer = 2;
    ...
    public void OnPickedUp()
    {
        ...
        /* Set the weapon's children's layers to "GunLayer" so that the gun
         * will not clip through objects (from the player's perspective). */
    ...
    public virtual void OnDropped()
    {
        ...
        /* Set the weapon's children's layers to 0 (default)
         * so that the player cannot see the weapon through objects. */
    ...
    public void OnMeleeInput()
    {
        ...
        if(Physics.Raycast (this.transform.position, this.transform.forward,
            out hit, this.meleeRange, ~0, QueryTriggerInteraction.Ignore))
        {
            hit.transform.GetComponent<Rigidbody>()?.AddForce(
                this.transform.forward * this.meleeForce);
